...
public T DivideToIntegerNaturalScale(
      ...
      if (helper.GetSign(ret) == 0) {
        ...
        if (desiredScale.Sign < 0) {
          ...
          while ((bigmantissa % (BigInteger)bigradix).IsZero) {
            bigmantissa /= bigradix;
    ...
    private T DivideInternal(
      ...
      if (signA == 0) {
        ...
        if (integerMode == IntegerModeFixedScale) {
          ...
          expdiff.CompareTo(ctx.Precision) > 0 &&
            desiredExponent.IsZero) {
        ...
        if (mantcmp == 0) {
          ...
          while (true) {
            ...
            BigInteger quotient = BigInteger.DivRem(mantissaDividend, mantissaDivisor, out remainder);
            ...
            if (remainder.IsZero && adjust.Sign >= 0) {
              break;
            }
