...
namespace PeterO.Numbers {
  ...
  internal sealed class SimpleRadixMath<T> : IRadixMath<T> {
    ...
    private T PostProcessEx(
      ...
      EInteger mant = EInteger.Abs(this.GetHelper().GetMantissa(thisValue));
      if (mant.IsZero) {
    ...
    private T RoundBeforeOp(T val, EContext ctx) {
      ...
      EInteger mant = EInteger.Abs(this.GetHelper().GetMantissa(val));
      FastInteger digits =
        this.GetHelper().CreateShiftAccumulator(mant).GetDigitLength();
    ...
    public T Power(T thisValue, T pow, EContext ctx) {
      ...
      if (powSign == 0 && this.GetHelper().GetSign(thisValue) == 0) {
        ...
        EInteger.Abs(this.GetHelper().GetMantissa(pow));
        {
