...
namespace PeterO.Cbor {
  /// <summary>Contains auxiliary methods that may have use outside the
  ...
  internal static class CBORUtilities {
    ...
    public static int CompareStringsAsUtf8LengthFirst(string strA, string
      ...
      // Console.WriteLine("slow path "+alen+","+blen);
      var sapos = 0;
      var sbpos = 0;
      long sautf8 = 0L;
      long sbutf8 = 0L;
    ...
    public static long IntegerToDoubleBits(int i) {
      ...
      /* Console.WriteLine("" + i + "->" + (longmant==DoubleToInt64Bits(i)));
      */ return longmant;
    }
    ...
    public static int DoubleToHalfPrecisionIfSameValue(long bits) {
      ...
      int sexp = exp - 1008;
      // Console.WriteLine("bits={0:X8}, exp=" + exp + " sexp=" + (sexp),bits);
      if (exp == 2047) { // Infinity and NaN
        ...
        // Console.WriteLine("mant=" + mant + " rs=" + (rs));
    ...
    public static bool DoubleRetainsSameValueInSingle(long bits) {
      ...
      long mant = bits & 0xfffffffffffffL;
      int sexp = exp - 896;
      // Console.WriteLine("sng mant={0:X8}, exp=" + exp + " sexp=" + (sexp));
