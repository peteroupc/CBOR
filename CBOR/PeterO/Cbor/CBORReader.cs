...
namespace PeterO.Cbor {
  ...
  newFirstByte) : this.ReadInternal();
    ...
    public async Task<CBORObject> ReadAsync() {
      CBORObject obj = this.options.AllowEmpty ?
           await this.ReadInternalOrEOFAsync() : await this.ReadInternalAsync();
      if (this.options.ResolveReferences && this.hasSharableObjects) {
        var sharedRefs = new SharedRefs();
        return ResolveSharedRefs(obj, sharedRefs);
      }
    ...
    private async Task<CBORObject> ReadInternalOrEOFAsync() {
      if (this.depth > 500) {
        throw new CBORException("Too deeply nested");
      }
      int firstbyte = await ReadByteAsync(this.stream);
      if (firstbyte < 0) {
        // End of stream
        return null;
      }
      return await this.ReadForFirstByteAsync(firstbyte);
    }

    private async Task<CBORObject> ReadInternalAsync() {
      if (this.depth > 500) {
        throw new CBORException("Too deeply nested");
      }
      int firstbyte = await ReadByteAsync(this.stream);
    ...
    public async Task<CBORObject> ReadForFirstByteAsync(int firstbyte) {
      if (this.depth > 500) {
        throw new CBORException("Too deeply nested");
      }
      if (firstbyte < 0) {
      ...
      if (expectedLength != 0) {
        ...
        data[0] = unchecked((byte)firstbyte);
        if (expectedLength > 1 &&
            await this.stream.ReadAsync(data, 1, expectedLength - 1) !=
expectedLength
            - 1) {
          throw new CBORException("Premature end of data");
        }
      ...
      if (type == 2) { // Byte string
        if (additional == 31) {
          ...
          using (var ms = new MemoryStream()) {
            // Requires same type as this one
            while (true) {
              int nextByte = await ReadByteAsync(this.stream);
              if (nextByte == 0xff) {
                // break if the "break" code was read
                break;
              }
              long len = await ReadDataLengthAsync(this.stream, nextByte, 2);
              if ((len >> 63) != 0 || len > Int32.MaxValue) {
                throw new CBORException("Length" + ToUnsignedEInteger(len) +
                  " is bigger than supported ");
              }
              if (nextByte != 0x40) {
                // NOTE: 0x40 means the empty byte string
                await ReadByteDataAsync(this.stream, len, ms);
              }
            }
            if (ms.Position > Int32.MaxValue) {
              throw new CBORException(
                 "Length of bytes to be streamed is bigger than supported ");
            }
            data = ms.ToArray();
            return CBORObject.FromRaw(data);
          }
          ...
          int hint = (uadditional > Int32.MaxValue || hasBigAdditional) ?
            Int32.MaxValue : (int)uadditional;
          data = await ReadByteDataAsync(this.stream, uadditional, null);
          return this.ObjectFromByteArray(data, hint);
        }
      }
      if (type == 3) { // Text string
        if (additional == 31) {
          ...
          while (true) {
            ...
            long len = await ReadDataLengthAsync(this.stream, nextByte, 3);
            if ((len >> 63) != 0 || len > Int32.MaxValue) {
              throw new CBORException("Length" + ToUnsignedEInteger(len) +
                " is bigger than supported");
            }
  ...
  false)) {
      ...
      if (type == 4) { // Array
        ...
        if (additional == 31) {
          ...
          // Indefinite-length array
          while (true) {
            int headByte = await ReadByteAsync(this.stream);
            if (headByte < 0) {
              throw new CBORException("Premature end of data");
            }
            if (headByte == 0xff) {
            ...
            ++this.depth;
            CBORObject o = await this.ReadForFirstByteAsync(headByte);
            --this.depth;
            cbor.Add(o);
            ++vtindex;
          }
        ...
        ++this.depth;
        for (long i = 0; i < uadditional; ++i) {
          cbor.Add(await this.ReadInternalAsync());
        }
        --this.depth;
        return cbor;
      }
      if (type == 5) { // Map, type 5
        ...
        if (additional == 31) {
          ...
          while (true) {
            ...
            ++this.depth;
            CBORObject key = await this.ReadForFirstByteAsync(headByte);
            CBORObject value = await this.ReadInternalAsync();
            --this.depth;
            if (!this.options.AllowDuplicateKeys) {
              if (cbor.ContainsKey(key)) {
                throw new CBORException("Duplicate key already exists: " + key);
        ...
        for (long i = 0; i < uadditional; ++i) {
          ++this.depth;
          CBORObject key = await this.ReadInternalAsync();
          CBORObject value = await this.ReadInternalAsync();
          --this.depth;
          if (!this.options.AllowDuplicateKeys) {
            if (cbor.ContainsKey(key)) {
      ...
      if (type == 6) { // Tagged item
        ...
        ++this.depth;
        CBORObject o = haveFirstByte ?
           await this.ReadForFirstByteAsync(newFirstByte) :
    ...
    private static async Task<byte[]> ReadByteDataAsync(
      Stream stream,
      long uadditional,
      Stream outputStream) {
      if ((uadditional >> 63) != 0 || uadditional > Int32.MaxValue) {
      ...
      if (uadditional <= 0x10000) {
        // Simple case: small size
        var data = new byte[(int)uadditional];
        if (await stream.ReadAsync(data, 0, data.Length) != data.Length) {
          throw new CBORException("Premature end of stream");
        }
        if (outputStream != null) {
          outputStream.Write(data, 0, data.Length);
        ...
        if (outputStream != null) {
          while (total > 0) {
            int bufsize = Math.Min(tmpdata.Length, total);
            if (await stream.ReadAsync(tmpdata, 0, bufsize) != bufsize) {
              throw new CBORException("Premature end of stream");
            }
            outputStream.Write(tmpdata, 0, bufsize);
            total -= bufsize;
    ...
    private static async Task<long> ReadDataLengthAsync(
      Stream stream,
      int headByte,
      int expectedType) {
      if (headByte < 0) {
      ...
      switch (headByte & 0x1f) {
        case 24: {
            if (await stream.ReadAsync(data, 0, 1) != 1) {
              throw new CBORException("Premature end of data");
            }
            return ((int)data[0]) & 0xff;
          }
        case 25: {
            if (await stream.ReadAsync(data, 0, 2) != 2) {
              throw new CBORException("Premature end of data");
            }
            int lowAdditional = ((int)(data[0] & (int)0xff)) << 8;
            lowAdditional |= (int)(data[1] & (int)0xff);
        ...
        case 26: {
            if (await stream.ReadAsync(data, 0, 4) != 4) {
              throw new CBORException("Premature end of data");
            }
            long uadditional = ((long)(data[0] & 0xffL)) << 24;
            uadditional |= ((long)(data[1] & 0xffL)) << 16;
        ...
        case 27: {
            if (await stream.ReadAsync(data, 0, 8) != 8) {
              throw new CBORException("Premature end of data");
            }
            // Treat return value as an unsigned integer
            long uadditional = ((long)(data[0] & 0xffL)) << 56;
