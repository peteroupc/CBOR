...
// import java.math.*;
  ...
  class RadixMath<T> {
    ...
    private T ReturnQuietNaN(T thisValue, PrecisionContext ctx) {
      ...
      if (mant.signum()!=0 && ctx != null && !ctx.Precision.signum()==0) {
        BigInteger limit = this.helper.MultiplyByRadixPower(
    ...
    private T MinMaxHandleSpecial(
      T thisValue, T otherValue, PrecisionContext ctx,
        ...
        if ((thisFlags & BigNumberFlags.FlagInfinity) != 0) {
          ...
          if (isMinOp) {
            return ((thisFlags & BigNumberFlags.FlagNegative) != 0) ?
              ...
              this.RoundToPrecision(otherValue, ctx);  // if positive, will be greater
          } else {
            return ((thisFlags & BigNumberFlags.FlagNegative) == 0) ?
              thisValue ://if positive, will be greater than every other number
              this.RoundToPrecision(otherValue, ctx);
          }
        }
        if ((otherFlags & BigNumberFlags.FlagInfinity) != 0) {
          if (compareAbs) {
          ...
          if (isMinOp) {
            return ((otherFlags & BigNumberFlags.FlagNegative) == 0) ?
              this.RoundToPrecision(thisValue, ctx) :
              otherValue;
          } else {
            return ((otherFlags & BigNumberFlags.FlagNegative) != 0) ?
              this.RoundToPrecision(thisValue, ctx) :
 ...
 throw new ArithmeticException("Division by zero");
    ...
    private boolean Round(
      IShiftAccumulator accum, Rounding rounding,
    ...
    private boolean RoundGivenDigits(
      int lastDiscarded, int olderDiscarded, Rounding rounding,
    ...
    private boolean RoundGivenBigInt(
      ...
      return this.RoundGivenDigits(
        accum.getLastDiscardedDigit(), accum.getOlderDiscardedDigits(), rounding,
        neg, bigval);
    }
    ...
    private T EnsureSign(T val, boolean negative) {
      ...
      if ((negative && (flags & BigNumberFlags.FlagNegative) == 0) ||
        ...
        return this.helper.CreateNewWithFlags(
          this.helper.GetMantissa(val), this.helper.GetExponent(val), flags);
      }
      return val;
    }
    ...
    public T DivideToIntegerNaturalScale(
      ...
      FastInteger desiredScale = FastInteger.FromBig(
        this.helper.GetExponent(thisValue)).SubtractBig(
        this.helper.GetExponent(divisor));
      ...
      if (this.helper.GetMantissa(ret).signum()==0) {
        ...
        ret = this.helper.CreateNewWithFlags(
          BigInteger.ZERO,
          dividendExp.subtract(divisorExp), this.helper.GetFlags(ret));
      } else {
...
bigrem=divrem[1]; }
          ...
          ret = this.helper.CreateNewWithFlags(
            bigmantissa, fastexponent.AsBigInteger(),
            this.helper.GetFlags(ret));
        }
      }
      if (ctx != null) {
    ...
    public T Abs(T value, PrecisionContext ctx) {
      ...
      if ((flags & BigNumberFlags.FlagNegative) != 0) {
        return this.RoundToPrecision(
          this.helper.CreateNewWithFlags(
            this.helper.GetMantissa(value), this.helper.GetExponent(value),
            flags & ~BigNumberFlags.FlagNegative),
          ctx);
      }
    ...
    public T Negate(T value, PrecisionContext ctx) {
      ...
      if ((flags & BigNumberFlags.FlagInfinity) == 0 && mant.signum()==0) {
        if ((flags & BigNumberFlags.FlagNegative) == 0) {
          ...
          return this.RoundToPrecision(
            this.helper.CreateNewWithFlags(
              mant, this.helper.GetExponent(value),
    ...
    private T NegateRaw(T val) {
      ...
      return this.helper.CreateNewWithFlags(
        this.helper.GetMantissa(val), this.helper.GetExponent(val),
    ...
    public T Pi(PrecisionContext ctx) {
      if (ctx == null || ctx.Precision.signum()==0) {
 throw new IllegalArgumentException("ctx is null or has unlimited precision");
      ...
      while (more) {
        ...
        T aMinusNewA = this.Add(a, this.NegateRaw(newA), null);
        if (!a.equals(b)) {
        ...
        if (more) {
          T tmpT = this.Multiply(aMinusNewA, aMinusNewA, null);
    ...
    private T PowerIntegral(T thisValue, BigInteger powIntBig, PrecisionContext ctx) {
      ...
      T one = this.helper.ValueOf(1);
      if (sign == 0)
        return this.RoundToPrecision(one, ctx);  // however 0 to the power of 0 is undefined
    ...
    private boolean IsWithinExponentRangeForPow(T thisValue, PrecisionContext ctx) {
      if (ctx == null || !ctx.getHasExponentRange())
        return true;
      FastInteger digits = this.helper.CreateShiftAccumulator((
    ...
    public T Power(T thisValue, T pow, PrecisionContext ctx) {
      ...
      if (!isPowIntegral) {
        powInt = this.Quantize(
          pow, this.helper.CreateNewWithFlags(
            BigInteger.ZERO,
            BigInteger.ZERO, 0),
      ...
      if ((!isPowIntegral || powSign < 0) &&  (ctx == null || ctx.Precision.signum()==0))
...
bigrem=divrem[1]; }
        ...
        if (cmpOne == 0) {
          ...
          if (this.compareTo(thisValue, quarter) <= 0) {
            ...
            thisValue = this.Multiply(
              thisValue, this.helper.CreateNewWithFlags(
                bigintRoots, BigInteger.ZERO, 0), ctxCopy);
          } else {
 ...
 throw new IllegalArgumentException("ctx is null or has unlimited precision");
      ...
      if ((flags & BigNumberFlags.FlagInfinity) != 0) {
        if ((flags & BigNumberFlags.FlagNegative) != 0) {
          T retval = this.RoundToPrecision(
            this.helper.CreateNewWithFlags(
 ...
 throw new IllegalArgumentException("ctx is null or has unlimited precision");
      ...
      if (this.helper.GetSign(thisValue) == 0) {
        ret = this.RoundToPrecision(
          this.helper.CreateNewWithFlags(
            BigInteger.ZERO, idealExp, this.helper.GetFlags(thisValue)), ctxtmp);
        if (ctx.getHasFlags()) {
      ...
      if (ctx.getHasFlags() && ctx.getClampNormalExponents() &&
          !this.helper.GetExponent(retval).equals(idealExp) &&
    ...
    public T NextMinus(
      ...
      if (ctx.Precision.signum() <= 0) { throw new IllegalArgumentException("ctx.getPrecision()" + " not less than " + "0" + " ("+ctx.getPrecision()+")"); }
      ...
      if ((flags & BigNumberFlags.FlagInfinity) != 0) {
        if ((flags & BigNumberFlags.FlagNegative) != 0) {
          ...
          BigInteger overflowMant = this.helper.MultiplyByRadixPower(
            BigInteger.ONE, FastInteger.FromBig(ctx.getPrecision()));
          overflowMant=overflowMant.subtract(BigInteger.ONE);
 ...
 throw new ArithmeticException("Rounding was required");
    ...
    private T DivideInternal(
      ...
      if (signB == 0) {
        ...
        return this.SignalDivideByZero(
          ctx,
          ((this.helper.GetFlags(thisValue) & BigNumberFlags.FlagNegative) != 0) ^
          ((this.helper.GetFlags(divisor) & BigNumberFlags.FlagNegative) != 0));
      }
      int radix = this.thisRadix;
      if (signA == 0) {
        T retval = null;
        if (integerMode == IntegerModeFixedScale) {
          int newflags = (this.helper.GetFlags(thisValue) & BigNumberFlags.FlagNegative) ^
            (this.helper.GetFlags(divisor) & BigNumberFlags.FlagNegative);
        ...
        boolean hasPrecision = ctx != null && ctx.Precision.signum() != 0;
        boolean resultNeg = (this.helper.GetFlags(thisValue) & BigNumberFlags.FlagNegative) !=
          (this.helper.GetFlags(divisor) & BigNumberFlags.FlagNegative);
...
rem=divrem[1]; }
            ...
            return this.helper.CreateNewWithFlags(
              quo, desiredExponent, resultNeg ?
              BigNumberFlags.FlagNegative : 0);
          } else if (ctx != null && ctx.Precision.signum() != 0 &&
...
rem=divrem[1]; }
        ...
        if (integerMode == IntegerModeRegular) {
          ...
          if ((mantissaDividend % mantissaDivisor).signum()==0) {
            ...
            return this.RoundToPrecision(
              this.helper.CreateNewWithFlags(
                quo, naturalExponent.AsBigInteger(), resultNeg ?
                BigNumberFlags.FlagNegative : 0), ctx);
          }
          if (hasPrecision) {
            ...
            dividendPrecision =
              this.helper.CreateShiftAccumulator(mantissaDividend).GetDigitLength();
            divisorPrecision =
              this.helper.CreateShiftAccumulator(mantissaDivisor).GetDigitLength();
            if (dividendPrecision.compareTo(divisorPrecision) <= 0) {
              ...
              divid = this.helper.MultiplyByRadixPower(
                divid, shift);
            } else {
...
rem=divrem[1]; }
            ...
            int[] digitStatus = this.RoundToScaleStatus(rem, mantissaDivisor, resultNeg, ctx);
            FastInteger natexp = FastInteger.Copy(naturalExponent).Subtract(shift);
            PrecisionContext ctxcopy = ctx.WithBlankFlags();
            T retval2 = this.RoundToPrecisionWithShift(
              this.helper.CreateNewWithFlags(
        ...
        if (mantcmp < 0) {
          ...
          mantissaDivisor = this.helper.MultiplyByRadixPower(
 ...
 throw new ArithmeticException("Rounding was required");
    ...
    public T MinMagnitude(T a, T b, PrecisionContext ctx) {
      ...
      return (cmp < 0) ? this.RoundToPrecision(a, ctx) :
    ...
    public T Max(T a, T b, PrecisionContext ctx) {
      ...
      if (flagNegA != (this.helper.GetFlags(b) & BigNumberFlags.FlagNegative)) {
        return (flagNegA != 0) ? this.RoundToPrecision(b, ctx) :
          this.RoundToPrecision(a, ctx);
      }
      if (flagNegA == 0) {
        return this.helper.GetExponent(a).compareTo(this.helper.GetExponent(b)) > 0 ? this.RoundToPrecision(a, ctx) :
          this.RoundToPrecision(b, ctx);
      } else {
        return this.helper.GetExponent(a).compareTo(this.helper.GetExponent(b)) > 0 ? this.RoundToPrecision(b, ctx) :
          this.RoundToPrecision(a, ctx);
      }
    }
    ...
    public T Min(T a, T b, PrecisionContext ctx) {
      ...
      if (signANeg != (this.helper.GetFlags(b) & BigNumberFlags.FlagNegative)) {
        return (signANeg != 0) ? this.RoundToPrecision(a, ctx) :
          this.RoundToPrecision(b, ctx);
      }
      if (signANeg == 0) {
        return this.helper.GetExponent(a).compareTo(this.helper.GetExponent(b)) > 0 ? this.RoundToPrecision(b, ctx) :
    ...
    public T Multiply(T thisValue, T other, PrecisionContext ctx) {
      ...
      T ret = this.helper.CreateNewWithFlags(
        this.helper.GetMantissa(thisValue).multiply(BigInteger.valueOf(this.helper)).GetMantissa(other), newexp,
        thisFlags)
        ;
      if (ctx != null) {
        ret = this.RoundToPrecision(ret, ctx);
    ...
    public T RoundToExponentSimple(
      ...
      if (this.helper.GetExponent(thisValue).compareTo(expOther) >= 0) {
        ...
        return this.RoundToPrecisionWithShift(
          this.helper.CreateNewWithFlags(bigmantissa, expOther, thisFlags), ctx,
...
bigrem=divrem[1]; }
    ...
    private T RoundToPrecisionInternal(
      ...
      if (ctx.Precision.signum()==0 && !ctx.getHasExponentRange() &&
      ...
      if (!binaryPrec) {
        ...
        if (fastPrecision.signum() > 0 &&
          ...
          if (digitCount.compareTo(fastPrecision) <= 0) {
            if (!this.RoundGivenDigits(
              lastDiscarded, olderDiscarded, ctx.getRounding(),
      ...
      if (adjustNegativeZero &&
          (thisFlags & BigNumberFlags.FlagNegative) != 0 && this.helper.GetMantissa(thisValue).signum()==0 &&
      ...
      if (binaryPrec) {
        ...
        IShiftAccumulator accumMaxMant = this.helper.CreateShiftAccumulator(
          maxMantissa);
        ...
        if (mantissaWasZero) {
          ...
          if (ctx.getClampNormalExponents()) {
            ...
            if (binaryPrec && this.thisRadix != 2) {
              fastPrecision = this.helper.CreateShiftAccumulator(maxMantissa)
                .GetDigitLength();
            }
 ...
 throw new ArithmeticException("Rounding was required");
          ...
          return this.helper.CreateNewWithFlags(
            newmantissa.AsBigInteger(), fastETiny.AsBigInteger(),
            neg ? BigNumberFlags.FlagNegative : 0);
        }
      }
 ...
 throw new ArithmeticException("Rounding was required");
        ...
        if (this.RoundGivenBigInt(accum, rounding, neg, bigmantissa)) {
          ...
          if (!unlimitedPrec &&
              (bigmantissa.testBit(0)==false || (this.thisRadix & 1) != 0) &&
      ...
      return this.helper.CreateNewWithFlags(
        bigmantissa, exp.AsBigInteger(),
        neg ? BigNumberFlags.FlagNegative : 0);
    }

    private T AddCore(
      BigInteger mant1,  // assumes mant1 is nonnegative
    ...
    public T Add(T thisValue, T other, PrecisionContext ctx) {
      ...
      if (expcmp == 0) {
          ...
          if (FastInteger.Copy(expdiff).compareTo(fastPrecision) > 0) {
            ...
            if (expcmp2 < 0) {
              if (op2MantAbs.signum()!=0) {
                ...
                //                    111111111111|
                //        222222222222222|
                FastInteger digitLength1 = this.helper.CreateShiftAccumulator(op1MantAbs)
                  .GetDigitLength();
                if (
                  ...
                  if (newDiff.compareTo(expdiff) < 0) {
                    ...
                    FastInteger digitLength2 = this.helper.CreateShiftAccumulator(
                    ...
                    if (digitLength2.compareTo(fastPrecision) < 0) {
                      ...
                      op2MantAbs = this.helper.MultiplyByRadixPower(
              ...
              if (op1MantAbs.signum()!=0) {
                ...
                if (
                  ...
                  if (newDiff.compareTo(expdiff) < 0) {
                    ...
                    digitLength2 = this.helper.CreateShiftAccumulator(
                    ...
                    if (digitLength2.compareTo(fastPrecision) < 0) {
                      ...
                      op1MantAbs = this.helper.MultiplyByRadixPower(
        ...
        if (expcmp > 0) {
          op1MantAbs = this.helper.RescaleByExponentDiff(
          ...
          op2MantAbs = this.helper.RescaleByExponentDiff(
    ...
    public T CompareToWithContext(T thisValue, T decfrac, boolean treatQuietNansAsSignaling, PrecisionContext ctx) {
      ...
      T result = this.CompareToHandleSpecial(thisValue, decfrac, treatQuietNansAsSignaling, ctx);
      if ((Object)result != (Object)null) return result;
    ...
    public int compareTo(T thisValue, T decfrac) {
      ...
      int flagsOther = this.helper.GetFlags(decfrac);
      ...
      int s = this.CompareToHandleSpecialReturnInt(thisValue, decfrac);
      if (s <= 1) { return s; }
      s = this.helper.GetSign(thisValue);
      int ds = this.helper.GetSign(decfrac);
      if (s != ds) { return (s < ds) ? -1 : 1; }
      if (ds == 0 || s == 0) {
        // Special case: Either operand is zero
        return 0;
      }
      int expcmp = this.helper.GetExponent(thisValue).compareTo((BigInteger)this.helper.GetExponent(decfrac));
      ...
      int mantcmp = (this.helper.GetMantissa(thisValue)).abs()
        .compareTo((this.helper.GetMantissa(decfrac)).abs());
      ...
      BigInteger op2Exponent = this.helper.GetExponent(decfrac);
      FastInteger fastOp1Exp = FastInteger.FromBig(op1Exponent);
      FastInteger fastOp2Exp = FastInteger.FromBig(op2Exponent);
      FastInteger expdiff = FastInteger.Copy(fastOp1Exp).Subtract(fastOp2Exp).Abs();
      // Check if exponent difference is too big for
      ...
      if (expdiff.CompareToInt(100) >= 0) {
        BigInteger op1MantAbs = (this.helper.GetMantissa(thisValue)).abs();
        BigInteger op2MantAbs = (this.helper.GetMantissa(decfrac)).abs();
        FastInteger precision1 = this.helper.CreateShiftAccumulator(
          op1MantAbs).GetDigitLength();
        FastInteger precision2 = this.helper.CreateShiftAccumulator(
          op2MantAbs).GetDigitLength();
        ...
        if (FastInteger.Copy(expdiff).compareTo(maxPrecision) > 0) {
          ...
          if (expcmp2 < 0) {
            if (op2MantAbs.signum()!=0) {
              ...
              //                    111111111111|
              //        222222222222222|
              FastInteger digitLength1 = this.helper.CreateShiftAccumulator(
                op1MantAbs).GetDigitLength();
              if (
                FastInteger.Copy(fastOp1Exp)
                .Add(digitLength1)
      ...
      if (expcmp > 0) {
        BigInteger newmant = this.helper.RescaleByExponentDiff(
          this.helper.GetMantissa(thisValue), op1Exponent, op2Exponent);
        BigInteger othermant = (this.helper.GetMantissa(decfrac)).abs();
        newmant = (newmant).abs();
        mantcmp = newmant.compareTo(othermant);
        return (s < 0) ? -mantcmp : mantcmp;
      } else {
        BigInteger newmant = this.helper.RescaleByExponentDiff(
          this.helper.GetMantissa(decfrac), op1Exponent, op2Exponent);
