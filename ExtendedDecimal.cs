...
namespace PeterO {
    ...
    /// same value.
    /// For example, 1.00 and 1 would be stored differently, even though
    /// they have
    /// the same value. In the first case, 100 * 10^-2 (100 with decimal
    /// point moved
    ...
    /// NaN,
    /// unless noted otherwise. Invalid operations will also return a quiet
    /// NaN, as
...
PrecisionContext ctx) {
      ...
      if (offset < 0) {
        throw new FormatException("offset (" +
          Convert.ToString(
(long)offset,
...
System.Globalization.CultureInfo.InvariantCulture) +
      ...
      int i = offset;
      if (i + 8 == endStr) {
        if ((str[i] == 'I' || str[i] == 'i') && (str[i + 1] == 'N' || str[i +
          1] == 'n') && (str[i + 2] == 'F' || str[i + 2] == 'f') &&
...
(negative ? BigNumberFlags.FlagNegative : 0) | BigNumberFlags.FlagSignalingNaN);
      ...
      (newScale == null) ? ((BigInteger)newScaleInt) :
...
flags);
    ...
    /// will be
    /// a multiple of 3. The format of the return value follows the format
    /// of the
...
PrecisionContext.ForRounding(Rounding.Down));
    ...
    /// rounding, and exponent range of the result. This context will be
    /// used only
...
null);
    ...
    /// (and can signal FlagInvalid and return NaN if the result
    /// doesn&apos;t fit
    ...
    /// FlagInvalid and returns NaN if the divisor and the dividend are 0.
    /// Signals
    /// FlagInvalid and returns NaN if the context defines an exponent range
    /// and the
...
newflags);
    ...
    /// <summary>Multiplies two decimal numbers. The resulting exponent will
    /// be the
...
ExtendedDecimal augend) {
    ...
    /// <param name='divisor'>The divisor.</param>
    /// <param name='ctx'>A precision context object to control the precision,
    /// rounding, and exponent range of the integer part of the result.
    /// Flags will
    ...
    /// exponent
    /// range. Signals FlagDivideByZero and returns infinity if the divisor
    /// is 0 and
...
PrecisionContext ctx) {
    ...
    /// from
    /// the operation (the flags are in addition to the pre-existing flags).
    /// Can be
    /// null.</param>
    ...
    /// returns
    /// NaN if the divisor and the dividend are 0, or if the result doesn't
    /// fit the
...
PrecisionContext ctx) {
    ...
    /// object&apos;s value. <list type='' ><item>If this and the other
    /// object divide
    /// evenly, the result is 0.</item>
    /// <item>If the remainder's absolute value is
    /// less than half of the divisor's absolute value, the result has the
    /// same sign
    ...
    /// rounding mode is always treated as HalfEven). If HasFlags of the
    /// context is
...
ExtendedDecimal second) {
    ...
    /// <para>If this
    /// object or the other object is a quiet NaN or signaling NaN, this
    /// method will
    ...
    /// exponent
    /// range are ignored. If HasFlags of the context is true, will store
    /// the flags
    /// resulting from the operation (the flags are in addition to the
    /// pre-existing
...
PrecisionContext ctx) {
    ...
    /// object is a quiet NaN or signaling NaN, this method will return a
    /// quiet NaN
...
PrecisionContext ctx) {
    ...
    /// precision. If rounding to a number of decimal places is desired,
    /// it's better
...
ctx);
    ...
    /// <summary>Returns a decimal number with the same value as this object but
    /// with the same exponent as another decimal number. <para>Note that
    /// this is
    ...
    /// example,
    /// -3 means round to the thousandth (10^-3, 0.0001), and 3 means round
    /// to the
    ...
    /// Signals FlagInvalid and returns NaN if the new exponent is outside
    /// of the
...
PrecisionContext ctx) {
    ...
    /// <summary>Returns a decimal number with the same value as this object but
    /// rounded to an integer, and signals an invalid operation if the
    /// result would
    /// be inexact.</summary>
    /// <param name='ctx' >A precision context to control precision and
    /// rounding of
    ...
    /// occurred, or the result can't fit the given precision without rounding.
    /// Signals FlagInvalid and returns NaN if the new exponent must be
    /// changed to 0
    ...
    /// 0.0001), and 3
    /// means round to the thousand (10^3, 1000). A value of 0 rounds the
    /// number to
    /// an integer.</param>
    /// <param name='ctx'>A PrecisionContext object.</param>
    /// <returns>A decimal number with the same value as this object but
...
PrecisionContext ctx) {
    ...
    /// <summary>Multiplies two decimal numbers. The resulting scale will be
    /// the sum
...
newflags);
    ...
    /// specified
    /// precision. Returns the same value as this object if <paramref
    /// name='ctx' />
  ...
  [Obsolete("Instead of this method, use RoundToPrecision and pass a precision context with the IsPrecisionInBits property set."
    ...
    /// number, but
    /// the return value is still NaN). Signals FlagInvalid and returns NaN
    /// if the
    ...
    /// with a real
    /// part equal to Ln of this object's absolute value and an imaginary
    /// part equal
    ...
    /// returns
    /// NaN if this object is less than 0. Signals FlagInvalid and returns
    /// NaN if
    ...
    /// <returns>This^exponent. Signals the flag FlagInvalid and returns NaN
    /// if this
