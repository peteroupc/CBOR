...
//--
    ...
    public void TestAsEInteger() {
      ...
      {
        string stringTemp =
          CBORObject.FromObject(0.75f).AsEInteger().ToString();
        Assert.AreEqual(
          "0",
          stringTemp);
      }
      {
        string stringTemp =
          CBORObject.FromObject(0.99f).AsEInteger().ToString();
        Assert.AreEqual(
          "0",
          stringTemp);
      }
      {
        string stringTemp =
          CBORObject.FromObject(0.0000000000000001f).AsEInteger()
                .ToString();
        Assert.AreEqual(
          "0",
          stringTemp);
      ...
      {
        string stringTemp = CBORObject.FromObject(0.5f).AsEInteger().ToString();
        Assert.AreEqual(
          "0",
          stringTemp);
      }
      {
        string stringTemp =
          CBORObject.FromObject(1.5f).AsEInteger().ToString();
        Assert.AreEqual(
          "1",
          stringTemp);
      }
      {
        string stringTemp =
          CBORObject.FromObject(2.5f).AsEInteger().ToString();
        Assert.AreEqual(
          "2",
          stringTemp);
      }
      ...
      {
        string stringTemp =
          CBORObject.FromObject(328323.0).AsEInteger().ToString();
        Assert.AreEqual(
          "328323",
          stringTemp);
      }
  ...
  EInteger.FromString("84170882933504200501581262010093"));
    ...
    public void TestEquivalentNegativeInfinity() {
      ...
      }
      {
        CBORObject objectTemp =
CBORObject.FromObject(CBORTestCommon.FloatNegInf);
        CBORObject objectTemp2 = CBORObject.FromObject(
...
CBORObject.FromObject(CBORTestCommon.FloatPosInf);
    ...
    public void TestTagArray() {
      ...
      {
        string stringTemp =
CBORObject.FromObject(obj.GetAllTags()).ToJSONString();
        Assert.AreEqual(
          "[999]",
          stringTemp);
      }
  ...
  EInteger.FromRadixString(
    ...
    public void TestReadJSON() {
      try {
        ...
        using (
          var msjson = new MemoryStream(new byte[] {
            0x74, 0x72, 0x75, 0x65,
          })) {
          Assert.AreEqual(CBORObject.True, CBORObject.ReadJSON(msjson));
        }
        ...
        using (
          var msjson = new MemoryStream(new byte[] { 0, 0, 0, 0x20, 0, 0 })) {
          try {
            CBORObject.ReadJSON(msjson);
            Assert.Fail("Should have failed");
          } catch (CBORException) {
        ...
        using (
          var msjson = new MemoryStream(new byte[] { 0, 0, 0xfe, 0xff, 0 })) {
          try {
            CBORObject.ReadJSON(msjson);
            Assert.Fail("Should have failed");
          } catch (CBORException) {
