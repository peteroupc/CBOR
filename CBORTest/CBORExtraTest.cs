...
// NOTE: Intentionally empty
      ...
      try {
        ToObjectTest.TestToFromObjectRoundTrip(CBORTestCommon.DecPosInf).AsDecimal();
        Assert.Fail("Should have failed");
      } catch (OverflowException) {
// NOTE: Intentionally empty
} catch (Exception ex) {
      ...
      try {
        ToObjectTest.TestToFromObjectRoundTrip(CBORTestCommon.DecNegInf).AsDecimal();
        Assert.Fail("Should have failed");
      } catch (OverflowException) {
// NOTE: Intentionally empty
} catch (Exception ex) {
...
// NOTE: Intentionally empty
      ...
      try {
        ToObjectTest.TestToFromObjectRoundTrip(CBORTestCommon.FloatPosInf).AsDecimal();
        Assert.Fail("Should have failed");
      } catch (OverflowException) {
// NOTE: Intentionally empty
} catch (Exception ex) {
      ...
      try {
        ToObjectTest.TestToFromObjectRoundTrip(CBORTestCommon.FloatNegInf).AsDecimal();
        Assert.Fail("Should have failed");
      } catch (OverflowException) {
// NOTE: Intentionally empty
} catch (Exception ex) {
...
from x in arrao select x;
 ...
 valueCcTT,
  ...
  "propC");
    ...
    public void TestArbitraryTypes() {
      CBORObject obj = ToObjectTest.TestToFromObjectRoundTrip(new { AByte.A, B = AInt.A, C =
                    AULong.A });
      if (obj == null) {
 Assert.Fail();
 }
 ...
 Assert.Fail();
    ...
    Assert.AreEqual(254, obj["a"].AsInt32());
      ...
      obj = ToObjectTest.TestToFromObjectRoundTrip((object)RangeExclusive(0, 10));
      Assert.AreEqual(10, obj.Count);
      Assert.AreEqual(0, obj[0].AsInt32());
      Assert.AreEqual(1, obj[1].AsInt32());
      CBORTestCommon.AssertRoundTrip(obj);
...
// NOTE: Intentionally empty
    ...
    public void TestOther() {
      CBORObject cbor = ToObjectTest.TestToFromObjectRoundTrip(new int[2, 3, 2]);
      {
  ...
  stringTemp);
    ...
    public void TestDecimal() {
      ...
      for (int i = -100; i <= 100; ++i) {
        ...
        {
CBORObject objectTemp = ToObjectTest.TestToFromObjectRoundTrip((decimal)i + 0.1m);
...
CBORTestCommon.AssertJSONSer(objectTemp, objectTemp2);
    ...
    public void TestDoubleToOther() {
      CBORObject dbl1 = ToObjectTest.TestToFromObjectRoundTrip((double)Int32.MinValue);
      CBORObject dbl2 = ToObjectTest.TestToFromObjectRoundTrip((double)Int32.MaxValue);
      try {
        dbl1.AsUInt16();
      } catch (OverflowException) {
// NOTE: Intentionally empty
