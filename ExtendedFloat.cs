...
namespace PeterO {
    ...
    /// NaN, even if another operand to that operation is a quiet NaN,
    /// unless noted
    /// otherwise.</para>
...
int flags) {
    ...
    /// consists
    /// of: <list type='' ><item>An optional '-' or '+' character (if '-' ,
    /// the value
    /// is negative.)</item>
...
newflags);
    ...
    /// handle a
    /// null return value by treating it as positive infinity if both
    /// operands have
    /// the same sign or as negative infinity if both operands have
    /// different signs.
...
ctx);
    ...
    /// flags). Can be null, in which case the default rounding mode is
    /// HalfEven.</param>
    /// <returns>A binary float with the same value as this object but
    /// rounded to an
    /// integer. Signals FlagInvalid and returns NaN if an overflow error
    /// occurred,
    /// or the result can't fit the given precision without rounding. Signals
    /// FlagInvalid and returns NaN if the new exponent must be changed to 0
    /// when
    /// rounding and 0 is outside of the valid range of the precision
    /// context, if it
    /// defines an exponent range.</returns>
    ...
    /// occurred,
    /// or the result can't fit the given precision without rounding. Signals
    /// FlagInvalid and returns NaN if the new exponent is outside of the valid
    ...
    /// <summary>Returns a binary float with the same value as this object, and
