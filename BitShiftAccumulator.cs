...
namespace PeterO {
 ...
 throw new ArgumentException("bits's sign (" + Convert.ToString((long)bits.Sign, System.Globalization.CultureInfo.InvariantCulture) + ") is less than " + "0");
    ...
    public BitShiftAccumulator(
      ...
      int olderDiscarded) {
      if (bigint.Sign < 0) {
 throw new ArgumentException("bigint's sign (" + Convert.ToString((long)bigint.Sign, System.Globalization.CultureInfo.InvariantCulture) + ") is less than " + "0");
}
    ...
    public static BitShiftAccumulator FromInt32(int smallNumber) {
      if (smallNumber < 0) {
 throw new ArgumentException("smallNumber (" + Convert.ToString((long)smallNumber, System.Globalization.CultureInfo.InvariantCulture) + ") is less than " + "0");
}
    ...
    public void ShiftToDigitsInt(int bits) {
      if (bits < 0) {
 throw new ArgumentException("bits (" + Convert.ToString((long)bits, System.Globalization.CultureInfo.InvariantCulture) + ") is less than " + "0");
}
      if (this.isSmall) {
