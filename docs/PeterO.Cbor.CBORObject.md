## PeterO.Cbor.CBORObject
 ...
 This class's natural ordering (under the CompareTo method) is consistent with the Equals method. This means that two values that compare as equal under the CompareTo method will also be equal under the Equals method (provided the value remains unchanged).
 ...
 Another consideration is that two values that are otherwise equal may have different tags. To strip the tags from a CBOR object before comparing, use the  `Untag`  method. To compare CBOR objects without regard to what tags they have, use CompareToIgnoreTags rather than CompareTo.
 ...
 One kind of CBOR object is called a map, or a list of key-value pairs. Keys can be any kind of CBOR object, including numbers, strings, arrays, and maps. However, untagged text strings (those objects for which GetTags returns an empty array and the Type property, or "getType()" in Java, returns CBORType.TextString) are the most suitable to use as keys; other kinds of CBOR object are much better used as map values instead, keeping in mind that some of them are not thread safe without synchronizing reads and writes to them.
...
The closest .NET decimal to this object.
...
### Write
 ...
 Writes a 64-bit floating-point number in CBOR format to a data stream. The number is written using the shortest floating-point encoding possible; this is a change from previous versions. To write the floating-point number using a given size (16, 32, or 64 bits), use the WriteFloatingPointValue method instead.
